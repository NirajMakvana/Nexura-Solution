...
import { taskValidator, validate, paginateQuery } from '../middleware/validate.js'
...
router.get('/', paginateQuery, async (req, res) => {
  try {
    ...
    res.json({ data: tasks, total, page, limit, pages: Math.ceil(total / limit) })
  } catch (error) {
    res.status(500).json({ message: error.message })
  }
})
...
// @desc    Create task
// @access  Private
router.post('/', taskValidator, validate, async (req, res) => {
  try {
