...
const EmployeeLoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // authService.login already persists to Zustand store
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-blue-50 to-indigo-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <p className="text-gray-600">Access your employee portal and manage your tasks</p>
        </div>

        <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8 border border-gray-100 mx-auto w-full">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label htmlFor="email" className="block text-sm font-medium text-gray-700 mb-2">Employee Email</label>
            ...
            <button type="submit" disabled={isLoading}
              ...
              {isLoading ? (
                <><Spinner variant="white" size={20} className="mr-2" />Signing In...</>
              ) : (
                <><User className="w-5 h-5 mr-2" />Access Employee Portal</>
              )}
            </button>
          </form>
        </div>
        ...
        <div className="text-center space-y-2">
          <Link to="/" className="text-gray-600 hover:text-gray-800 text-sm font-medium transition-colors block">
            ← Back to Website
          </Link>
        </div>
